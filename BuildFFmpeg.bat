...
:: -Settings passed but no file specified
if !SettingsFound!==Y (
    goto Usage
)
...
if defined MSYS2_BIN (
    if exist %MSYS2_BIN% goto GetSettings
)
...
:: Get the custom settings if parameter was set
:GetSettings
if %BUILD.settings%==N (
    set BUILD.settingsString=""
) else (
    echo Using settings %BUILD.settings%

    for /f "tokens=*" %%F IN ('powershell -ExecutionPolicy Bypass ..\Get-FFmpegBuildSettings.ps1 %BUILD.settings%') DO (
        set BUILD.settingsString=%%F
    )

    echo Build settings !BUILD.settingsString!
)
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win10 x86 "%BUILD.settingsString%"
endlocal
goto Cleanup
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win10 x64 "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win10 ARM "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win8.1 x86 "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win8.1 x64 "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win8.1 ARM "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Phone8.1 ARM "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Phone8.1 x86 "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win7 x86 "%BUILD.settingsString%"
endlocal
...
%MSYS2_BIN% --login -x %~dp0FFmpegConfig.sh Win7 x64 "%BUILD.settingsString%"
endlocal
...
echo:
echo     %0 [target platform] [architecture] [[-Settings] file.xml]
echo:
echo where
echo:
echo [target platform] is: win10 ^| win8.1 ^| phone8.1 ^| win7 (at least one)
echo [architecture]    is: x86 ^| x64 ^| ARM (optional)
echo [-Settings]       is: (optional) xml file containing a FFmpegCodecPack element
echo:
echo For example:
